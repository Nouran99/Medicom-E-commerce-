...
productsEnhancedRoutes.get('/', async (c) => {
  try {
    ...
    if (search) {
      ...
      if (lang === 'ar') {
        query = query.or(`name_ar.ilike.${searchTerm},active_ingredient.ilike.${searchTerm},product_code.ilike.${searchTerm}`);
      } else {
        query = query.or(`name_en.ilike.${searchTerm},active_ingredient.ilike.${searchTerm},product_code.ilike.${searchTerm}`);
      }
    }
