...
productsRoutes.get('/', async (c) => {
  try {
    ...
    let query = supabase
      .from('products')
      .select('*, categories(name_en, name_ar), providers(name_en, name_ar), pickup_locations(name_en, name_ar, address_en, address_ar)')
      .eq('in_stock', true);
...
productsRoutes.get('/search', async (c) => {
  try {
    ...
    if (lang === 'ar') {
      dbQuery = dbQuery.or(`name_ar.ilike.${searchTerm},description_ar.ilike.${searchTerm},sku.ilike.${searchTerm}`);
    } else {
      dbQuery = dbQuery.or(`name_en.ilike.${searchTerm},description_en.ilike.${searchTerm},sku.ilike.${searchTerm}`);
    }
