...
supplierRoutes.get('/api/admin/suppliers', async (c) => {
  try {
    ...
    // Apply search filter
    if (search) {
      query = query.or(`name.ilike.%${search}%,seller_code.ilike.%${search}%,contact_info->email.ilike.%${search}%`);
    }
...
supplierRoutes.get('/api/admin/suppliers/:id', async (c) => {
  try {
    ...
    const stats = {
      ...
      totalInventoryValue: products?.reduce((sum, p) => sum + (p.stock_quantity * p.price_per_unit), 0) || 0,
...
supplierRoutes.put('/api/admin/suppliers/:id', async (c) => {
  try {
    ...
    if (updates.email || updates.phone || updates.address || updates.city || updates.country || updates.website || updates.contact_person) {
  ...
  try {
    ...
    const supplierStats = suppliers?.map(supplier => {
      ...
      return {
        ...
        total_inventory_value: supplierProducts.reduce((sum, p) => sum + (p.stock_quantity * p.price_per_unit), 0),
        total_units: supplierProducts.reduce((sum, p) => sum + p.stock_quantity, 0)
      };
    }).sort((a, b) => b.product_count - a.product_count).slice(0, 10);
